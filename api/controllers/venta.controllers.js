...
ventaCTRL.deleteVenta = async (req, res) => {
  ...
  try {
    const ventaEliminada = await Venta.findByIdAndUpdate(
      id,
      { eliminada: true },
      { new: true },
    );
...
ventaCTRL.getForMes = async (req, res) => {
  ...
  try {
    ...
    const inicioMes = new Date(
      `${anio}-${mesActual.toString().padStart(2, "0")}-01T00:00:00-03:00`,
    );
    const finMes = new Date(
      `${siguienteMes > 12 ? anio + 1 : anio}-${(siguienteMes % 13 || 1).toString().padStart(2, "0")}-01T00:00:00-03:00`,
    );
...
ventaCTRL.postOne = async (req, res) => {
  const { listaProductos, tipo_comp, imprimirCliente, F, dispositivo, mesa } =
    req.body;
