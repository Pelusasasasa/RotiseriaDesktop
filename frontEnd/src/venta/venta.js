...
let mesaURL = getParameterByName("mesa");
...
//Guaradamos la mesa que tragaimos al buscar
let mesa;
...
const calcularTotal = async () => {
  ...
  if (cantidadEmapandas > 0) {
    ...
    total +=
      docenas * precioEmpanadas.docena +
      medias * precioEmpanadas.mediaDocena +
      sueltas * precioIndivudial;
  }
...
const cambiandoObservaciones = async () => {
  if (mesa) {
    mesa.observaciones = observaciones.value;
    await axios.put(`${URL}mesa/${mesaURL}`, mesa);
  }
};
...
const cambiarSituacion = (situacion) => {
  situacion === "negro"
    ? tarjetaCredito.classList.add("none")
    : tarjetaCredito.classList.remove("none");
};
...
const listarCliente = async (id) => {
  ...
  if (cliente !== "") {
    ...
    if (mesa) {
      mesa.cliente = cliente.nombre;
      mesa.idCliente = cliente._id;
...
const listarMesa = (mesa) => {
  ...
  listarProductos(mesa.productos);

  facturar.innerHTML = `<button class="w-90">Cerrar Mesa</button>`;
};
...
const listarProductos = async (lista) => {
    ...
    div.innerHTML += `
      ...
      producto.descripcion
                ...
                <div class='flex flex-col flex-1' id='${producto._id}'>
                    <h4 class='m-0 text-xs font-semibold text-balance'>${
                      producto.descripcion
                    }</h4>
  ...
  setTimeout(() => {
    divCarrito.scrollTop = divCarrito.scrollHeight;
  }, 0);
...
const quitarElemento = async (id) => {
  ...
  if (mesa) {
    mesa.productos = [...carrito.productos];
    await axios.put(`${URL}mesa/${mesa._id}`, mesa);
  }
  calcularTotal();
};

const restarElemento = async (id) => {
  ...
  const idProducto = carrito.productos[index].producto._id;

  const span = document.getElementById(`${idProducto}`).children[2].children[0]
    .children[1];
...
const sumarElemento = async (id) => {
  const index = carrito.productos.findIndex((elem) => elem.producto._id == id);
  carrito.productos[index].cantidad += 1;

  const idProducto = carrito.productos[index].producto._id;

  //Tomamos el span que tiene el numero para actualizar la cantidad
  const span = document.getElementById(`${idProducto}`).children[2].children[0]
    .children[1];
  span.innerText = parseInt(span.innerText) + 1;
...
window.addEventListener("load", async (e) => {
  ...
  if (mesaURL) {
    ...
    const { data } = await axios.get(`${URL}mesa/${mesaURL}`);
    if (data.ok) {
      mesa = data.mesa;
      listarMesa(mesa);
    } else {
      await Swal.fire("Erro al botener la mesa", data.msg, "error");
    }
  }
});
...
facturar.addEventListener("click", async (e) => {
  ...
  venta.tipo_comp =
    situacion === "blanco"
      ? await verTipoComprobante(venta.cod_comp)
      : "Comprobante";

  const [iva21, iva0, gravado21, gravado0, iva105, gravado105, cantIva] =
    await sacarIva(carrito.productos); //  acamos el iva de los productos
  venta.iva21 = iva21;
  venta.iva0 = iva0;
  ...
  if (isConfirmed) {
    const { value: precioEnvio, isConfirmed } = await sweet.fire({
      input: "text",
      title: "Precio del envio",
      inputPlaceholder: "Ingrese el precio del envio",
    });
    if (!isConfirmed) {
      venta.precioEnvio = 0;
    }
    venta.imprimirCliente = true;
    venta.precioEnvio = precioEnvio;
  } else {
