...
pestaña.addEventListener("click", async (e) => {
  if (e.target.parentNode.nodeName === "MAIN") {
    document.querySelector(".pestañaSeleccionada") &&
      document
        .querySelector(".pestañaSeleccionada")
        .classList.remove("pestañaSeleccionada");
...
window.addEventListener("load", async (e) => {
  ...
  try {
    ...
    if (!data.ok)
      return await Swal.fire("Error al trear variables", data.msg, "error");
...
const eliminarVenta = async (e) => {
  ...
  const { isConfirmed } = await sweet.fire({
    title:
      target.children[3].innerText === "Factura C"
        ? "¿Hacer una Nota de Credito?"
        : "¿Seguro queres Eliminar La venta?",
    confirmButtonText: "Aceptar",
    showCancelButton: true,
  });
...
const verQueTraer = async () => {
  if (botonSeleccionado.classList.contains("botonDia")) {
    if (filtro === "Ingresos") {
      ...
      presupuestos = (
        await axios.get(`${URL}presupuesto/forDay/${fecha.value}`)
      ).data;
      return presupuestos;
    } else {
...
botonDia.addEventListener("click", async (e) => {
  ...
  if (filtro === "Ingresos" || filtro === "Cuenta Corriente") {
    ...
    presupuestos = (await axios.get(`${URL}presupuesto/forDay/${fecha.value}`))
      .data;
...
const listarVentas = async (ventas) => {
  ...
  for await (let venta of lista) {
    ...
    const tdPrecioEnvio = document.createElement("td");
    ...
    if (eliminadas) {
      ...
      buttonAccion.innerText =
        venta.tipo_comp === "Factura C" ? "assignment" : "";
    } else {
    ...
    if (venta.listaProductos) {
      for await (let { cantidad, producto } of venta.listaProductos) {
        if (producto) {
          ...
          tdIdProducto.innerText =
            producto._id === undefined ? " " : producto._id;
    ...
    totalVenta +=
      venta.tipo_comp === "Nota Credito C" ? venta.precio * -1 : venta.precio;
    totalVenta += venta.precioEnvio ?? 0;
  }
...
document.addEventListener("keyup", (e) => {
  if (
    e.key === "Escape" &&
    !document.activeElement.classList.contains("swal2-confirm")
  ) {
    location.href = "../menu.html";
  }
});
